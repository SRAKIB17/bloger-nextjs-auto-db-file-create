...
const DogEmoji = ({ props: { selectEmoji, setSelectEmoji } }) => {
    ...
    return (
        <div>
            <div className={styles?.emoji}>
                {
                    [...Array(8).keys()].map((index) => < EmojiShowImg index={index} key={index} selectEmoji={selectEmoji} selectEmojiHandle={selectEmojiHandle} />)
                }
            </div>
        </div>
    );
...
const EmojiShowImg = ({ index, selectEmoji, selectEmojiHandle }) => {
    ...
    const images = require.context('../../../../public/emojis/dogEmoji/', true);
