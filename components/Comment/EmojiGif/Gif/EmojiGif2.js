...
const EmojiGif2 = ({ props: { selectEmoji, setSelectEmoji } }) => {
    ...
    return (
        <div>
            <div className={styles?.emojiGif}>
                {
                    [...Array(34).keys()].map((index) => < EmojiShowImg index={index} key={index} selectEmoji={selectEmoji} selectEmojiHandle={selectEmojiHandle} />)
                }
            </div>
        </div>
    );
...
const EmojiShowImg = ({ index, selectEmoji, selectEmojiHandle }) => {
    ...
    const images = require.context('../../../../public/gif/emoji 2', true);
    let emojiGif = images(`./emoji (${index + 1}).gif`);
    return (
        <div>
            <span
