...
const CoverPhoto = ({ setCoverThumbnail, coverThumbnail }) => {
    const { fileData, uploadFileHandler, message, result, uploading } = useProfilePicUploader({ dropdownId: "#uploaderManuallyCoverPhoto" });

    useEffect(() => {
        // setThumbnailData(fileData);
        setCoverThumbnail(result?.data?.url)
    }, [fileData, result])
    ...
    const hightLightHandle = event => {
        ...
        event.target.ownerDocument.querySelector('#uploaderCoverPhotoFile').classList.add('highlight')
    }
    const unHightLightHandel = event => {
        event.preventDefault()
        event.stopPropagation()
        event.target.ownerDocument.querySelector('#uploaderCoverPhotoFile').classList.remove('highlight')
    }
    return (
        <div>
                ...
                <div>
                    ...
                    <div className='relative'>
                        <div
                            ...
                            onDrop={imageDrop} id="uploaderCoverPhotoFile">
                    ...
                    <div>
                        <label className="  btn btn-primary btn-sm text-white mb-2">
                            <input
                                type="file" name="image_file" id="uploaderManuallyCoverPhoto"
