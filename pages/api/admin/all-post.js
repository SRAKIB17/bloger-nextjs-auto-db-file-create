...
export default async function handler(req, res) {
    ...
    if (checkUser && roll && method === 'GET') {
        ...
        const findAllPost = await postCollection.find({}).sort({ _id: -1 }).skip(0).limit(parseInt(show)).toArray()
        return res.status(200).json({ message: "success", result: findAllPost })
    }
